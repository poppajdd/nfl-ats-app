...
def add_ats_cover_label(df):
    def cover(row):
        if pd.isna(row.get('spread')) or pd.isna(row.get('home_score')) or pd.isna(row.get('away_score')):
            return np.nan
        return int(row['home_score']-row['away_score']>row['spread'])
